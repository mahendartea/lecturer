...
class PtnsNegeri extends Component
{
    use WithPagination;
    public $paginate = 10;
    public $search;
    ...
    {
        if ($this->search == null) {
            return view('livewire.admin.ptns.ptns-negeri', ['ptns' => Dataptn::where('statuspt', 1)->paginate($this->paginate)]);
        } else {
            return view('livewire.admin.ptns.ptns-negeri', ['ptns' => Dataptn::where('statuspt', 1)->where('nama_universitas', 'like', '%' . $this->search . '%')->paginate($this->paginate)]);
        }
    }
