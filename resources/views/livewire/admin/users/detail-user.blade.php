<div>
  <h1 class="text-center text-gray-600 text-lg">Detail User</h1>
  <hr>
  <div class="my-5">
    <div class="max-w-sm mx-auto overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
      ...
      <div class="flex items-center px-6 py-3 bg-gray-900">
        ...
        @switch($role_id)
        @case(1)
        <h1 class="mx-3 text-lg font-semibold text-white">Administrator</h1>
        @break
        @case(2)
        <h1 class="mx-3 text-lg font-semibold text-white">Student</h1>
        @break
        @case(3)
        <h1 class="mx-3 text-lg font-semibold text-white">Lecturer</h1>
        @break
        @endswitch
      ...
      <div class="px-6 py-4">
        ...
        <p class="py-2 text-gray-700 dark:text-gray-400 text-sm font-bold">
          {{ $student_licence_number ? 'Nomor Induk Mahasiswa (' . $student_licence_number . ')' : '' }}
          {{ $teacher_nidn ? 'Nomor Induk Dosen / NIDN (' . $teacher_nidn . ')' : '' }}
        </p>
        ...
        <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
          ...
          <h1 class="px-2 text-sm">{{ $institution }}</h1>
        </div>
        ...
        <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
          ...
          <h1 class="px-2 text-sm">{{ $student_address }}</h1>
        </div>
        ...
        <p class="my-5 text-xs italic float-right">Last Update :
          {{ \Carbon\Carbon::parse($updated_at)->format('M, d Y') }}
        </p>
      </div>
    </div>
  </div>
